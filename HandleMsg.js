...
const mess = {
            wait: '[ ESPERA ] Estamos haciendo el traabajo⏳',
            error: {
...
module.exports = HandleMsg = async (aruga, message) => {
        ...
        if (isGroupMsg && GroupLinkDetector && !isGroupAdmins && !isOwnerBot){
            if (chats.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    aruga.reply(from, '*[ANTI LINK]*\nOh valla.. el anti link esta activado, y veo que enviaste un enlace valido... Adios', id).then(() => {
	...
	// Filter Banned People
        ...
        break
            ...
            case 'antistiker':
                    ...
                    case 'listblock':
                        ...
                        break
                    case 'antilink':
                    if (!isGroupMsg) return aruga.reply(from, 'Solo se puede hacer en grupos', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Lo siento, solo administradores pueden activar esto', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Error, el bot no es administrador ;)', id)
                    if (args[0] == 'on') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            ...
                            aruga.reply(from, '*[Anti Group Link]* Elimino a personas que pasen links!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(!cek){
       ...
       case 'profile':
        case 'mystat':{
            ...
            var adm = admins
            if (ct == null) {
                return await aruga.reply(from, 'Error interno [ hubo un error al conectarse con la base de datos de whatsapp ]', id) 
            } else {
        ...
        case 'help':
            ...
            .then(() => ((isGroupMsg) && (isGroupAdmins)) ? aruga.sendText(from, `Menu Admin: *${prefix}menuadmin*`) : null)
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'Uh.. Esto solo se puede hacer en grupos', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Uh.. No eres admin..', id)
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Aqui tienes el numero de mi creador:'))
            break
        case 'join':
            if (!isOwnerBot) return aruga.reply(from, `Este comando ha sido restringido`, id)
            ...
            if (!islink) return aruga.reply(from, 'Link invalido', id)
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          await aruga.sendText(from, 'Me he unido!')
                          await aruga.sendText(chekgrup.id, `Hola.n/soy LoL bot, no olvides darme admin y ver mis comandos con ${prefix}menu :)`)
                      })
            } else {
            ...
            case 'addsay':{
             if (isBanned) return false
                ...
                const say = body.slice(8)
                    ...
                    aruga.reply(from, `Conseguido!\nEl numero de tu /say es : *${dsay.length - 1}* ,` , id)
            }
            break
            case 'admins':
                if (!isGroupMsg) return aruga.reply(from, 'Solo se puede hacer en grupos', id)
                ...
                break
            case 'miembros':
                if (!isGroupMsg) return aruga.reply(from, 'Solo se puede hacer en grupos')
                const tulul = name
                const yaelah = chat.groupMetadata.participants.length
                    await aruga.sendText(from, `La cantidad de miembros en *${tulul}* es de : *${yaelah}*` )
                    break
            case 'owner':
                ...
                break          
            case 'setpic':
                    if (!isOwnerBot) return aruga.reply(from, `Este comando, esta reservado para el creador`, id)
                    if (isMedia) {
    ...
    case 'say':
        ...
        if(args.length == 1){
            ...
            if(sayur <= wuh) return await aruga.reply(from, `Mi base de datos: *${sayur - 1}` , id)
            const lahs = dsay[wuh]
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                const mediaData = await decryptMedia(quotedMsg)
                aruga.reply(from, `Espera, esto tarda...`, id)
                const imageBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', 'De nada, perra!', id)
                .then(() => {
                    console.log(`Sticker to Image Processed for ${processTime(t, moment())} Seconds`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Formato invalido, asegurate de responder el sticker con el comando!`, id)
        break
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[Trabajando] Espera⏳ Este proceso tarda ± 1 min!, (si no hace el sticker, prueba de nuevo)', id)
                    ...
                    await exec(`gify ${filename} ./media/stickergf.gif --fps=30 --scale=240:240`, async function (error, stdout, stderr) {
                        ...
                        .catch(() => {
                            aruga.reply(from, 'Hubo un error!', id)
                        })
                    })
                  } else {
                    aruga.reply(from, `[❗] Error, El comando *${prefix}stickergif* Solo permite gifs/videos menos de 10 sec!`, id)
                   }
                } else {
		    aruga.reply(from, `[❗] Formato incorrecto *${prefix}stickergif*`, id)
	        }
            break
        case 'stikergiphy':
        case 'stickergiphy':
            if (args.length !== 1) return aruga.reply(from, `Uso rapido \nTe hago stickers de giphy ${prefix}stickergiphy <link_giphy>`, id)
        ...
        case 'meme':
            if ((isMedia || isQuotedImage) && args.length >= 2) {
                ...
                aruga.sendFile(from, ImageBase64, 'image.png', '', null, true)
                    .then(() => {
                        aruga.reply(from, 'Hecho!',id)
                    })
                    .catch(() => {
                        aruga.reply(from, 'Hubo un error')
                    })
            } else {
                await aruga.reply(from, `Sin imagen/texto envie el comando asi ${prefix}meme <Texto uno> | <texto>\nEjemplo: ${prefix}meme Lol bot | Es lo mejor`, id)
            }
            break
        case 'quotemaker':
            ...
            break
        case 'nulis':
            if (args.length == 0) return aruga.reply(from, `Arto de usar cuaderno para anotar todo?\nPues: ${prefix}nulis [texto]\n\nTiene la solucion!`, id)
            const nulisq = body.slice(7)
            const nulisp = await rugaapi.tulis(nulisq)
            await aruga.sendImage(from, `${nulisp}`, '', 'Eh..', id)
            .catch(() => {
                aruga.reply(from, 'Hubo un error', id)
            })
            break
...
*➸ Se llama: : ${groupname}*
Este grupo fue fundado el *${date}* A las *${time}* Por @${ownerwoi.replace('@c.us','')}
*➸ Miembros : ${totalMem}*
*➸ Descripcion* 
${desc}
₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋₋
_Desc actualizado por : @${chat.groupMetadata.descOwner.replace('@c.us','')} el *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* A las ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
	...
	case 'grouplink':
            ...
            if (isGroupMsg) {
                ...
                aruga.sendLinkWithAutoPreview(from, inviteLink, `\nEste es el link del grupo *${name}* Para cambiar el link con *${prefix}revoke*`)
            } else {
            	aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            }
            break
	case "revoke":
 	if (!isGroupAdmins) return aruga.reply(from, 'Solo administradores pueden hacer esto', id)
	if (!isBotGroupAdmins) return aruga.reply(from, 'Lo siento, no cuento con admin para cambiar el link del grupo', id)
                    if (isBotGroupAdmins) {
                        aruga
                            .revokeGroupInviteLink(from)
                            .then((res) => {
                                aruga.reply(from, `Cambiado! para ver el nuevo link con *${prefix}grouplink*`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        case 'ytmp3':
			...
			rugaapi.ytmp3(`https://youtu.be/${linkmp3}`)
                     aruga.sendPtt(from, './lib/test.mp3', id)
            break
        case 'ytmp4':
            if (args.length == 0) return aruga.reply(from, `Untuk mendownload lagu dari youtube\nketik: ${prefix}ytmp3 [link_yt]`, id)
            ...
            .then(async(res) => {
				...
				await aruga.sendFileFromUrl(from, `${res.result.fragment_base_url}`, '', '', id)
				.catch(() => {
	...
	if (!isNsfw) return aruga.reply(from, 'Lo siento marrano, pidelea tu admin marrano que habilite el nsfw :)', id)
        ...
        // Random Kata
        case 'screen': {
            if (!isOwnerBot) return await aruga.reply(from, 'Fitur ini hanya dapat digunakan oleh admin bot')
